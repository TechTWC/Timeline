...
- Explore map-tile requests must remain disabled until the user explicitly enables Explore external map/POI access; moving or zooming the local map before consent must not produce an external request.
- Explore POI lookups may send only an explicitly user-selected map bounding box to the configured OpenStreetMap/Overpass provider, and only after the user presses the current-area search action. Raw Timeline points, semantic visits, and reconstructed history must never be included in those requests.
